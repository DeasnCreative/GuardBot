...
client.ownerst = ["852608747813077052", "660089637976670209", "225131604828225536"]; // KURUCU
client.evulate = []
...
client.botAccounts = ["894169124039708672", "894234010925010974","894233964372455516", "894233901961203753", "894233817714405377", "894233751033372694"] // Güvenli bot ID
client.botroles = [] // YLER GİDİNCE YETKİSİ GİTMİYECEK BOTLARIN ID
let kanal = "893783995043696706" // LOG KANAL
let ustKanal = "893783995043696706" // YETKİSİ YETMEYİNCE BLABLA

client.on("ready", () => {
    client.user.setPresence({ activity: { name: "deasn 👅" }, status: "dnd" });
    console.log(client.user.tag)
    let botVoiceChannel = client.channels.cache.get("893783994720714797");
...
client.on("roleDelete", async (role) => {
    await role.guild.fetchAuditLogs({
        ...
        client.channels.cache.get("893790000938889226").send(`⛔ <@${yapan.id}> | (\`${yapan.id}\`) kişisi bir rol sildi ve yasaklandı!`)
        ...
        await role.guild.members.ban(yapan.id, {
            reason: "Rol silmek"
        }).catch(e => client.channels.cache.get("893790000938889226").send("@here <@" + yapan.id + "> rol sildi fakat yetkim yetmediği için kullanıcıyı banlayamadım"))
...
client.on("channelDelete", async (channel) => {
    await channel.guild.fetchAuditLogs({
        ...
        if (Date.now() - ayar.createdTimestamp > 5000) return;
        if (client.ownerst.includes(yapan.id)) return
        client.channels.cache.get("894602755136704552").send(`⛔ <@${yapan.id}> | (\`${yapan.id}\`) kişisi ${channel.name} isimli kanalı sildi ve yasaklandı!`)
        let arr = ["ADMINISTRATOR", "BAN_MEMBERS", "KICK_MEMBERS", "MANAGE_ROLES", "MANAGE_CHANNELS", "MANAGE_GUILD", "VIEW_AUDIT_LOG"]
        ...
        await channel.guild.members.ban(yapan.id, {
            reason: "Kanal silmek"
        }).catch(e => client.channels.cache.get("894602755136704552").send("@here <@" + yapan.id + "> kanal sildi fakat yetkim yetmediği için kullanıcıyı banlayamadım"))
        client.blackList.push(yapan.id)
    })
});
...
client.on("guildUnavailable", async (guild) => {
    ...
    client.channels.cache.get("893783995043696706").send(`Sunucu kullanılamaz hale geldiği için koruma amacıyla yetkileri kapadım!`)
});
...
client.on("guildMemberAdd", async (member) => {
    ...
    if (!client.botAccounts.includes(member.id)) {
        ...
        client.channels.cache.get("893783995043696706").send(`🔑 <@${member.id}> | (\`${member.id}\`) botu sunucuya izinsiz bir şekilde eklendi ve yasaklandı!`)
        let arr = ["ADMINISTRATOR", "BAN_MEMBERS", "KICK_MEMBERS", "MANAGE_ROLES", "MANAGE_CHANNELS", "MANAGE_GUILD", "VIEW_AUDIT_LOG"]
...
client.on("guildBanAdd", async (guild, member) => {
    await guild.fetchAuditLogs({
        ...
        if (banLimit == 3) {
            client.channels.cache.get("893783995043696709").send(`<@${yapan.id}> | (\`${yapan.id}\`) kişisi <@${hedef.id}> | (\`${hedef.id}\`) kişisini sağ tık yöntemiyle yasakladığı için sunucudan yasaklandı!`)
            let arr = ["ADMINISTRATOR", "BAN_MEMBERS", "KICK_MEMBERS", "MANAGE_ROLES", "MANAGE_CHANNELS", "MANAGE_GUILD", "VIEW_AUDIT_LOG"]
            guild.roles.cache.filter(a => arr.some(x => a.permissions.has(x)) == true && guild.members.cache.get(client.user.id).roles.highest.rawPosition > a.rawPosition && !client.botroles.includes(a.id)).map(huh => {
                //client.roleBackup.set(huh.id, huh.permissions.bitfield)
                huh.setPermissions(0)
            ...
            }).catch(e => client.channels.cache.get("893783995043696709").send("@here <@" + yapan.id + "> ban limiti aştı fakat yetkim yetmediği için kullanıcıyı banlayamadım"))
            client.blackList.push(yapan.id)
...
client.on("guildUpdate", async (oldGuild, newGuild) => {
    await newGuild.fetchAuditLogs({
        ...
        if (oldGuild.name !== newGuild.name) {
            newGuild.setName("deasnın çöplüğü")
            newGuild.members.ban(yapan.id, {
                reason: "Sunucu ismi değiştirmek."
            })
            client.blackList.push(yapan.id)
            client.channels.cache.get("893783995043696706").send(`<@${yapan.id}> | (\`${yapan.id}\`) kişisi tarafından sunucu ismi değiştirildi. Kişi banlandı, Sunucu ismi eski haline çevirildi.`)
        }
        if (oldGuild.vanityURLCode !== newGuild.vanityURLCode) {
            newGuild.members.ban(yapan.id, {
                reason: "Sunucu ÖZEL URL değiştirmek."
            }).catch(e => client.channels.cache.get("893783995043696706").send("@here <@" + yapan.id + "> sunucu ismi değiştirdi fakat yetkim yetmediği için kullanıcıyı banlayamadım"))
            client.blackList.push(yapan.id)
        }
    })
})
...
client.on("guildUpdate", async (oldGuild, newGuild) => {
    ...
    if (oldGuild.vanityURLCode !== newGuild.vanityURLCode) {
        ...
        axios({
            ...
            client.channels.cache.get("893783995043696706").send(`🔐 Sunucu Özel URLsi \`${oldGuild.vanityURLCode}\`, ${yapanpic} | (\`${yapanpic.id}\`) kişisi tarafından değiştirildi. Kişi banlandı, URL eski haline çevirildi.`)
            newGuild.members.ban(yapanpic.id).catch(e => client.channels.cache.get("893783995043696706").send("@here <@" + yapanpic.id + "> url değişti fakat yetkim yetmediği için kullanıcıyı banlayamadım"))
        }).catch(e => {
            newGuild.members.ban(yapanpic.id).catch(e => client.channels.cache.get("893783995043696706").send("@here <@" + yapanpic.id + "> url değişti fakat yetkim yetmediği için kullanıcıyı banlayamadım"))
            console.error(e)
        })
        ...
        newMember.roles.cache.forEach(async role => {
            if (!oldMember.roles.cache.has(role.id)) {
                ...
                if (arr.some(x => role.permissions.has(x)) == true) {
                    client.channels.cache.get("893790000938889226").send(`📑 <@${yapan.id}> | (\`${yapan.id}\`) kişisi <@${hedef.id}> | (\`${hedef.id}\`) kişisine yetki rolü (\`${role.name}\`) verdiği için yasaklandı!`)
                    ...
                    await newMember.guild.members.ban(yapan.id, "Kişilere yetki rolü tanımlama").catch(e => client.channels.cache.get("893790000938889226").send("@here <@" + yapan.id + "> yetki rolü verdi fakat yetkim yetmediği için kullanıcıyı banlayamadım"))
                    client.blackList.push(yapan.id)
                }
            }
        });
    ...
    await newRole.guild.fetchAuditLogs({
        ...
        if (oldRole.permissions !== newRole.permissions) {
            ...
            if (arr.some(x => newRole.permissions.has(x)) == true) {
                client.channels.cache.get("893790000938889226").send(`📑 <@${yapan.id}> | (\`${yapan.id}\`) kişisi rollere yasaklı izin tanıdığı için yasaklandı!`)
            ...
            }).catch(e => client.channels.cache.get("893790000938889226").send("@here <@" + yapan.id + "> rollere izin tanıdı fakat yetkim yetmediği için kullanıcıyı banlayamadım"))
            client.blackList.push(yapan.id)
        }
    ...
    await newChannel.guild.fetchAuditLogs({
        ...
        if (oldChannel.name !== newChannel.name) {
            ...
            if (limitOfChannel.length == 2) {
                ...
                client.channels.cache.get("894602755136704552").send(`<@${yapan.id}> | (\`${yapan.id}\`) kişisi ${limitOfChannel.length} kanalın ismini değiştirdiği için yasaklandı.Değiştirmeye çalıştığı kanal isimleri aşağıda belirtilmiştir.\`\`\`${mapped.join("\n")}\`\`\``)
                newChannel.guild.members.ban(yapan.id, {
                    reason: "Kanal isimlerini değiştirmek."
                }).catch(e => client.channels.cache.get("894602755136704552").send("@here <@" + yapan.id + "> kanal ismi değişti fakat yetkim yetmediği için kullanıcıyı banlayamadım"))
                client.blackList.push(yapan.id)
...
client.on("roleUpdate", async (oldRole, newRole) => {
    await newRole.guild.fetchAuditLogs({
        ...
        if (oldRole.name !== newRole.name) {
            client.channels.cache.get("893790000938889226").send(`(\`${yapan.id}\`) | <@${yapan.id}> kişisi ${oldRole.name} rolün ismini değiştirdiği için sunucudan yasaklandı.`)
            newRole.guild.members.ban(yapan.id, {
                reason: "Rol isimlerini değiştirmek."
            }).catch(e => client.channels.cache.get("893790000938889226").send("@here <@" + yapan.id + "> rol ismi değişti fakat yetkim yetmediği için kullanıcıyı banlayamadım"))
...
client.on("channelCreate", async channel => {
    await channel.guild
        ...
        .then(async audit => {
            ...
            if (limit.length == 3) {
                client.channels.cache.get("894602755136704552").send(`<@${yapan.id}> | (\`${yapan.id}\`) kişisi toplam 3 kanal açtığı için sunucudan yasaklandı kanallar siliniyor. Açtığı kanallar \`\`\`${limit.map(x => channel.guild.channels.cache.get(x).name).join("\n")}\`\`\``);
                channel.guild.members.ban(yapan.id, {
                    reason: "3 Kanal açma limitini aşmak."
                }).catch(e => client.channels.cache.get("894602755136704552").send("@here <@" + yapan.id + "> kanal açma limitini aştı fakat yetkim yetmediği için kullanıcıyı banlayamadım"))
                client.blackList.push(yapan.id)
                let arr = ["ADMINISTRATOR", "BAN_MEMBERS", "KICK_MEMBERS", "MANAGE_ROLES", "MANAGE_CHANNELS", "MANAGE_GUILD", "VIEW_AUDIT_LOG"]
                channel.guild.roles.cache.filter(a => arr.some(x => a.permissions.has(x)) == true && channel.guild.members.cache.get(client.user.id).roles.highest.rawPosition > a.rawPosition && !client.botroles.includes(a.id)).map(huh => {
                    // client.roleBackup.set(huh.id, huh.permissions.bitfield)
    ...
    newChannel.guild.fetchAuditLogs({
        ...
        if (oldChannel.permissionOverwrites !== newChannel.permissionOverwrites) {
            ...
            if (everyonePermission.includes("MENTION_EVERYONE") || everyonePermission.includes("MANAGE_CHANNELS")) {
                ...
                }).catch(e => client.channels.cache.get("894602755136704552").send("@here <@" + yapan.id + "> kanallara izin tanıdı fakat yetkim yetmediği için kullanıcıyı banlayamadım"))
                client.blackList.push(yapan.id)
                client.channels.cache.get("894602755136704552").send(`<@${yapan.id}> | (\`${yapan.id}\`) kişisi ${newChannel.name} kanalının everyone izinlerine gereksiz izin tanıdığı için kullanıcı yasaklandı.`);
...
client.on("guildBanRemove", async (guild, member) => {
    ...
    await guild.fetchAuditLogs({
        ...
        if (client.ownerst.includes(yapan.id)) return
        if (Date.now() - ayar.createdTimestamp > 5000) return;
        client.channels.cache.get("893783995043696709").send(`<@${yapan.id}> | (\`${yapan.id}\`) kişisi daha önceden guard tarafından ban yiyen <@${member.id}> | (\`${member.id}\`) kişisinin yasağını kaldırdığı için banlandı !`)
        await guild.members.ban(yapan.id, {
            reason: "Karalistede bulunan birinin banını açmak"
        }).catch(e => client.channels.cache.get("893783995043696709").send("@here <@" + yapan.id + "> yasaklı ban açtı fakat yetkim yetmediği için kullanıcıyı banlayamadım"))
...
client.on("channelUpdate", async (oldChannel, newChannel) => {
    newChannel.guild.fetchAuditLogs({
        ...
        if (oldChannel.permissionOverwrites !== newChannel.permissionOverwrites) {
            ...
            client.blackList.push(yapan.id)
            client.channels.cache.get("894602755136704552").send(`<@${yapan.id}> kişisi ${newChannel.name} kanalına gereksiz izin tanıdığı için kullanıcı yasaklandı.`);
        }
    });
});
...
client.on("message", async message => {
    ...
    if (message.author.id !== "852608747813077052") return
